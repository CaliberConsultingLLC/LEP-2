...
//   reach  how far the bird reaches from each edge of its box, taken from the
//          crown down in ten steps, per unit of its drawn height
//
...
// Where the measurable copies live. The app ships WebP — this reads PNG alpha
// and nothing else — so the two are kept side by side under art-source/ and
// answer to the same name once the extension is stripped. Overridable so a new
// drop can be measured before it is anything the app knows about.
const ART_DIR = path.resolve(
  process.argv.find((a) => a.startsWith('--dir='))?.slice(6)
  ?? path.join(ROOT, 'art-source', 'measure'),
);
...
function measure(file) {
  ...
  // How far the bird reaches from each edge of its box, taken from the crown
  // down in ten steps, against its own drawn HEIGHT rather than the picture.
  //
  // A bird standing in a corner beside a column of the page has to be capped
  // by its width, and the only way to cap a height by a width is a ratio. The
  // whole bounding box is the wrong ratio to use, because a standing guide is
  // mostly BELOW whatever it stands beside: the reading's card ends a couple
  // of hundred pixels above the floor, and the part of the bird level with it
  // is head and shoulders, not the book and the spread of the tail. Capping on
  // the full box took a third off the Summary's guide to clear a card its
  // widest point was nowhere near.
  //
  // REACH, not width. The bird is anchored by one edge of its box and the
  // question is how far the top of it gets from there — which is not the same
  // as how wide the top is, because the top of the bird is not centred over
  // the bottom of it. Measuring the width instead let Mother's shoulders sit
  // 87px inside the reading while the arithmetic said they fitted.
  //
  // Both edges, because which one is the anchor depends on the corner: a bird
  // standing on the left is mirrored, so its LEADING edge in the picture is
  // the one the corner holds. This file knows nothing about mirroring, so it
  // writes both and the consumer picks.
  //
  // Ten steps rather than one number because the curve between them is not a
  // line — the real profile bulges where the wings come out, above anything
  // read straight from the half-height figure. Each step is non-decreasing by
  // construction, so a consumer that rounds UP to the next step is always
  // conservative.
  const reachL = [];
  const reachR = [];
  {
    ...
    for (let step = 1; step <= 10; step += 1) {
      const until = y0 + Math.round((bodyH * step) / 10) - 1;
      for (; row <= Math.min(until, y1); row += 1) {
        for (let x = 0; x < width; x += 1) {
          if (alpha[row * width + x] > ALPHA_FLOOR) {
            if (x < px0) px0 = x;
            if (x > px1) px1 = x;
          }
        }
      }
      const r3 = (v) => Math.round((v / bodyH) * 1000) / 1000;
      reachL.push(px1 < 0 ? 0 : r3(px1 - x0 + 1));
      reachR.push(px1 < 0 ? 0 : r3(x1 - px0 + 1));
    }
  }
...
for (const file of files) {
  ...
  anchors[key] = { box: m.box, face: m.face, speak: m.speak, branch: m.branch, reach: m.reach };
...
//
// Measured from ${rows.length} files in ${path.relative(ROOT, ART_DIR).split(path.sep).join('/')}.
`;

const body = Object.entries(anchors)
  .map(([k, v]) => `  '${k}': { box: [${v.box.join(', ')}], face: [${v.face.join(', ')}], speak: [${v.speak.join(', ')}], branch: '${v.branch}', reach: { l: [${v.reach.l.join(', ')}], r: [${v.reach.r.join(', ')}] } },`)
  .join('\n');
...
// crash.
export const DEFAULT_ANCHOR = { box: [0.07, 0.05, 0.94, 0.96], face: [0.33, 0.05, 0.66, 0.3], speak: [0.5, 0.11], branch: 'right', reach: { l: [0.62, 0.64, 0.66, 0.7, 0.78, 0.82, 0.88, 0.9, 0.93, 0.93], r: [0.62, 0.64, 0.66, 0.7, 0.78, 0.82, 0.88, 0.9, 0.93, 0.93] } };
