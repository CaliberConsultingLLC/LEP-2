...
if (useCairnTheme && typeof document !== 'undefined') {
  ...
  if (!alreadyLoaded) {
    ...
    fontLink.href =
      'https://fonts.googleapis.com/css2?family=Cinzel:wght@500;600;700&family=Fraunces:ital,opsz,wght@0,9..144,300..700;1,9..144,300..700&family=Manrope:wght@400;500;600;700;800&family=JetBrains+Mono:wght@400;500;600;700&display=swap';
...
try {
  if (!sessionStorage.getItem('compassVisitTracked')) {
    fetch('/api/track-visit', {
      ...
      // Marked only once it lands, so a failed ping is retried on the next load.
      .then((res) => { if (res.ok) sessionStorage.setItem('compassVisitTracked', '1'); })
      .catch(() => {});
  }
} catch {
